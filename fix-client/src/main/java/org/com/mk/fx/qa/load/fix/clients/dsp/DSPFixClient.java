...
public class DSPFixClient implements AutoCloseable {
  ...
  public DSPFixClient(
      ...
      String portsConfiguration,
      String environment,
      Set<String> clients) {
    try {
      this.clientPoolManager =
          new QFClientPoolManager(
              "client-configs/dsp/demo-client-cfg.json",
              "ports.json",
              "TEST",
              Set.of("trap_client"));
      this.clientPoolManager.startAll();
      clientPoolManager.setGlobalMessageListener(
          (sessionId, message) ->
              log.info("Received message: {} for session: {}", message, sessionId));
      clientPoolManager.setGlobalOutboundMessageListener(
          (sessionId, message) -> log.info("Sent message: {} for session: {}", message, sessionId));
      clientPoolManager.setGlobalSessionEventListener(
